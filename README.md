...
Бот автоматически проверяет здоровье TURN. Интервал настраивается через `.env`:

```env
CHECK_INTERVAL_MIN_MINUTES=5   # минимум (по умолчанию 5)
CHECK_INTERVAL_MAX_MINUTES=10  # максимум (по умолчанию 10)
```

При `min === max` — фиксированный интервал без рандома. При `min > max` — бот не запустится с ошибкой. По умолчанию — рандом 5–10 минут (для обхода паттернов детекции). Первая проверка — через 30 секунд после старта.
