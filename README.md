...
- Публикует ссылку в закреплённый пост закрытой VK-группы (роутер забирает по крону)
...
VK relay (опционально — для публикации ссылки в группу, чтобы роутер забирал по крону):
...
# ID закреплённого поста (создай пост в группе вручную, скопируй ID)
VK_POST_ID=1
```
...
2. **VK** — обновляет закреплённый пост в VK-группе через `wall.edit` (если настроен)
...
## VK relay (публикация для роутера)

Бот публикует ссылку в закреплённый пост закрытой VK-группы. Роутер забирает ссылку по крону через `wall.get` и перезапускает vk-turn-client.

```
Telegram → бот → wall.edit → VK-группа → роутер (крон) → wall.get → перезапуск клиента
```

VK relay опционален. Если переменные `VK_GROUP_TOKEN`, `VK_GROUP_ID`, `VK_POST_ID` не заданы — ссылка сохраняется только локально.

### Настройка VK relay

1. Создать закрытую группу в VK
2. Получить токен сообщества: Управление → Работа с API → Создать ключ (права: стена)
3. Создать пост в группе (любой текст) и скопировать его ID из URL
4. Заполнить в `.env`:
...
5. Перезапустить бота: `systemctl restart vk-turn-bot`
