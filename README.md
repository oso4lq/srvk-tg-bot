...
3. **Ссылка** — если на VPS установлен vk-turn-client и задан `VPS_PUBLIC_IP`, бот запускает loopback-тест: клиент пытается установить TURN-сессию через VK и вернуться на сервер по публичному IP. Если ссылка мертва — клиент падает с ошибкой до таймаута. При каждой проверке клиент подключается под случайным латинским названием рыбы (Salmo, Esox, Silurus и т.д.)
